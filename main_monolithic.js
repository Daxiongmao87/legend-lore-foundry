Hooks.once('ready', () => {
    ...
    console.log("legend-lore | ProseMirror and ProseMirrorHighlightMatchesPlugin found.");
                    ...
                    tooltipInstance.update = async function(view, lastState) {
                      ...
                      async function callOpenAI(preprompt, contextPrompt) {
                        ...
                        const requestOptions = {
                          ...
                          body: JSON.stringify({
                            model: game.settings.get('legend-lore', 'model') , // Replace with your model of choice
                            messages: [
                              {
                                role: "system",
    ...
    console.log('legend-lore | ProseMirrorHighlightMatchesPlugin build method overridden.');
});

Hooks.on('renderModuleSettings', (app, html, data) => {
    // Check if this is your module's settings
    if (data.module === 'legend-lore') {
        // Load your HTML template
        fetch('modules/legend-lore/templates/settings.html')
            ...
            .then(template => {
                ...
                html.find('#openai-api-key').val(game.settings.get('legend-lore', 'openaiApiKey'));

                // Add event listener for form submission
                html.on('submit', 'form', async (event) => {
                    event.preventDefault();
                    const apiKey = html.find('#openai-api-key').val();
                    await game.settings.set('legend-lore', 'openaiApiKey', apiKey);
                });
            });
    }
});
...
async function updateModelDropdown(html) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            defaultModel=(game.settings.get('legend-lore', 'model')) ? game.settings.get('legend-lore', 'model') : "gpt-3.5-turbo"
            ...
            if (html) {
              console.log(html.find('[data-setting-id="legend-lore.model"]').find('[class="notes"]')[0].innerHTML);
              ...
              dropdown = html.find('[name="legend-lore.model"]');
